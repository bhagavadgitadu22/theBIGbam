...
use crate::cigar::{Cigar, CigarOp, MdTag};
...
pub fn process_read(
    ...
    // Update covered bitmap (always needed for coverage percentage)
    let cov_start = raw_start.min(ref_length);
    let cov_end = raw_end.min(ref_length);
    for i in cov_start..cov_end {
        arrays.covered[i] = true;
    }
    ...
    // Parse CIGAR once for all uses
    let cigar = Cigar::from_raw(cigar_raw);

    // Phagetermini processing
    if flags.phagetermini {
        let check_end = seq_type.is_long();

        // Check if read starts/ends with match using CIGAR and MD tag
        let start_matches = cigar_starts_with_match(&cigar, md_tag, true);
        let end_matches = !check_end || cigar_starts_with_match(&cigar, md_tag, false);
    ...
    if flags.assemblycheck {
        ...
        // Insert sizes and bad orientations (short-paired)
        if seq_type.is_short_paired() {
        ...
        if !cigar.is_empty() {
            if cigar.first().map(|e| e.op.is_clipping()).unwrap_or(false) {
                arrays.left_clippings[start] += 1;
            }
            if cigar.last().map(|e| e.op.is_clipping()).unwrap_or(false) {
        ...
        if let Some(md_bytes) = md_tag {
            ...
            for pos in md.count_mismatches(raw_start, ref_length) {
...
/// Check if CIGAR starts/ends with a match (helper for phagetermini).
#[inline]
fn cigar_starts_with_match(cigar: &Cigar, md: Option<&[u8]>, at_start: bool) -> bool {
