...
When multiple isoforms share the same locus tag, only the **main isoform** — the first mRNA seen in file order and the CDS rows linked to it via `Parent_annotation_id` — is used for codon analysis. All isoforms are still displayed in the gene map, but codon/amino acid annotations are derived from the main one only. This simplifies visualization by avoiding conflicting or redundant annotations at overlapping positions.
