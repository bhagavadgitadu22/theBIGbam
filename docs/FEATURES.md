...
| -------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Primary alignments   | Primary reads         | The number of reads covering each position, counting only primary alignments. CIGAR `N` (skip) spans — introduced by spliced RNA-seq alignments across introns — are **subtracted** from this count, because the read carries no sequence across the skipped region. Mismatches (`X`), insertions (`I`), and deletions (`D`) are **not** subtracted: in those cases the read still aligns across the reference position (a mismatch is a base call, a deletion is a gap the read spans, an insertion sits between two aligned bases), so the position is genuinely covered. Only `N` represents a true skip where the read provides no information at intermediate positions (see the [RNA module](#rna-module)) | Depth of coverage. For RNA-seq, the intronic drop between exons is preserved rather than filled in by spliced reads |
...
Features specific to spliced alignments from RNA-seq data. Populated whenever reads contain CIGAR `N` (skip) operations; for DNA/prokaryotic data the track is empty.

| Subplot   | Feature   | Description                                                                                                                                                                                                                     | Use case                                                                                                                                                                                                |
| --------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Splicings | Splicings | Per-base count of CIGAR `N` (skip) operations from primary alignments. At each reference position, counts how many primary reads skip over that base via an `N` op — i.e. how many spliced reads have an intron covering here. | Visualise intron locations and splice-junction support in RNA-seq data; distinguish true introns (rectangular splicings block, matching drop in coverage) from deletions (which do not affect coverage) |
