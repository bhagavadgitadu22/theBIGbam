...
| Other alignments     | Secondary             | Reads flagged as secondary (SAM flag 0x100). Per the SAM specification, a secondary alignment is an alternative mapping when a read aligns to multiple genomic locations. In theory, secondary alignments represent full-length alternative placements of the read. **In practice**, aligners like minimap2 and bwa-mem2 may also report **partial** secondary alignments with soft clipping, where only a portion of the read matches an alternative location. This occurs when a mapping chain scores above the aligner's reporting threshold (e.g., minimap2's `-p` parameter, default 0.8, controls the minimum secondary-to-primary score ratio). | High secondary alignment counts indicate repetitive or ambiguous regions where reads could map to multiple places |
...
**Warning about `--circular` mode:** The `--circular` mapping option doubles the contig length during mapping so that reads can align across the circular junction. This produces ghost secondary and supplementary alignments (the same read mapping to equivalent positions in both copies of the doubled reference). The `convert_circular_bam` step filters these ghosts: a secondary or supplementary alignment is discarded if its aligned interval is fully contained within any primary alignment's interval in circular coordinates. After ghost removal, if no real secondary or supplementary alignments remain for a read, its MAPQ is set to 60 (the maximum for minimap2/bwa-mem2) since the low MAPQ was solely an artifact of the doubled reference.
