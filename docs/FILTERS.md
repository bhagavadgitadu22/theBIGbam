...
**Warning:** To work properly, the [classification protocol](docs/PHAGE_PACKAGING.md) needs the `--blast` flag to be enabled to have access to the repeats per contig (DTR and ITR). Some packaging mechanisms could be misclassified or completely missed otherwise.
